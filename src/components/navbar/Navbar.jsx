  ...
  return (
    ...
    `}>
      ...
      <div className="some-div flex items-center justify-center space-x-1 md:space-x-2">
        ...
        <div ref={navigationRef} className="hamburger-div flex flex-col cursor-pointer justify-center items-center space-y-[0.15rem] w-[1.5rem]  p-1" onClick={() => setNav((prev) => !prev)}>
        ...
        <ul id="navigation-dropdown" className={`nav-inside-div flex flex-col text-sm w-full md:w-[320px] top-[2.55rem] absolute ${nav ? "h-[214px] border py-3" : "h-0 py-0 border-none"} transition-height overflow-hidden duration-500 rounded-md right-0 px-6 z-30 ${!darkMode ? "bg-white text-black border-[#d3d3d3]" : "bg-[#212529] text-white border-white"}`}>
          ...
          <NavLink className="w-full border-t border-inherit py-2" to={"/dashboard"} onClick={()=>setNav(false)}>Dashboard</NavLink>
          <NavLink className="w-full border-y border-inherit py-2" to={"/profile"} onClick={()=>setNav(false)}>Profile</NavLink>
